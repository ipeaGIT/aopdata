...
#'}
read_traveltime <- function(city='bel', mode = 'walk', peak = TRUE, year = 2019, geometry = FALSE, showProgress = TRUE){

  # Get metadata with data url addresses
  temp_meta <- select_metadata(t='traveltime',
                               c=city,
                               y=year,
                               m=mode)
  ...
  if(geometry == FALSE){
                        ...
                        # return sf
                        aop_landuse <- read_access(city=city, mode=mode, peak=peak, year=year, showProgress=showProgress, geometry=FALSE)

                        # create function aop_join to bring in land use info
                        aop <- aop_join(aop_df, aop_sf)
                        return(aop)
                        }
  }
